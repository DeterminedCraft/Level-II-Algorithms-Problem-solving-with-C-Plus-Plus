...
/*  Mental Model:
   ...
   The following utility is used:

   void AddNumberToArray(int number, int array[], int &ArrayLength)   // the array is automatically passed by ref.
 {
	array[ArrayLength] = number;
	ArrayLength++;
 }

 Notice that, for-loop is not used becasue ArrayLength is unknown as the number of elements in the array is dependent
 on the user answering with yes or no.

 Hence, ArrayLength is used as the index of the array (has to be initialized to 0 in main()),
 where it's, thereafter, IMMEDIATELY incremented by 1 to reflect the current number of elements
 in the array after adding an element. This also serves as the next index of the array shall 
 the user decide to add another element,

 Note (still verify): enums are not used in this mental model.
...
/*
 ...
 {
	array[ArrayLength] = number;   // Always strive to populate the array starting from index 0, GENERALLY SPEAKING & in this mental model as well.
	ArrayLength++;
...
void AddElementToArray(int number, int array[], int& ArrayLength)   // the array is automatically passed by ref.
{
	array[ArrayLength] = number;   // Always strive to populate the array starting from index 0, GENERALLY SPEAKING & in this utility as well.
	ArrayLength++;
